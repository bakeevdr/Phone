!(function($) {
        ...
        validateByAjax: function($form, data) {
            if (data.status === 'ok') {
                ...
                if (data.code === 'main') {
                    ...
                    $errField.focus();
					$mainErrorContainer.removeClass('hide')
                }
            }
    ...
    function tr(params, lang) {
            ...
            messages.ru_RU = {
                'error': {
                    'form-submit': 'submit Произошла ошибка при отправке формы',
                    'ajax-request': 'request Произошла ошибка при отправке запроса'
                },

                'notice': {
                    'confirm': 'confirm Подтвердите действие'
                }
            };
            lang = lang || 'ru_RU';
